...
use crate::{
    ...
    wutil::{
        file_id_for_fd, file_id_for_path, wgettext, wperror, wrealpath, wrename, wstat, wunlink,
    ...
    pub fn matches_search(&self, term: &wstr, typ: SearchType, case_sensitive: bool) -> bool {
        ...
        match typ {
            ...
            SearchType::ContainsGlob => {
                ...
                wildcard_match(&content_to_match, &pat, false)
            }
...
impl HistoryImpl {
        ...
        for i in 0..MAX_SAVE_TRIES {
            ...
            if !can_replace_file {
                ...
                if wrename(&tmp_name, &target_name) == -1 {
                    FLOGF!(
                        error,
                        wgettext!("Error when renaming history file: %s"),
                        errno::errno()
                    );
                }
    ...
    fn disable_automatic_saving(&mut self) {
        self.disable_automatic_save_counter
            .checked_add(1)
            .expect("overflow");
    }
        ...
        if src_fd.is_valid() {
            ...
            while let Ok(n) = src_fd.read(&mut buf) {
                ...
                if dst_fd.write(&buf).is_err() {
                    ...
                    FLOGF!(history_file, "Error when writing history file");
                    break;
                }
            }
        }
    ...
    fn get_history(&mut self, result: &mut Vec<WString>) {
        ...
        if let Some(file_contents) = &self.file_contents {
            for &offset in &self.old_item_offsets {
    ...
    unsafe fn maybe_lock_file(fd: RawFd, lock_type: libc::c_int) -> bool {
        ...
        if let Ok(duration) = start_time.elapsed() {
            if duration > Duration::from_millis(250) {
                FLOGF!(
                    warning,
                    wgettext!("Locking the history file took too long (%.3f seconds)."),
...
pub fn expand_and_detect_paths<P: IntoIterator<Item = WString>>(
    ...
    let ctx = OperationContext::new(vars, EXPANSION_LIMIT_BACKGROUND);
...
fn rust_expand_and_detect_paths(
    ...
    expand_and_detect_paths(paths.from_ffi(), Arc::clone(&vars.0) as _).to_ffi()
}
...
fn in_private_mode_ffi(vars: &EnvDynFFI) -> bool {

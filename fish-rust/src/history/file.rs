...
use super::{history_ffi::PersistenceMode, history_never_mmap, HistoryItem};
...
impl HistoryFileContents {
    ...
    pub fn create(fd: RawFd) -> Option<Self> {
        ...
        if !mmap_file_directly {
            ...
            read_from_fd(fd, region.as_mut()).ok()?;
        }
...
fn read_from_fd(fd: RawFd, dest: &mut [u8]) -> std::io::Result<()> {
    todo!()
}

fn replace_all(s: &mut String, needle: &str, replacement: &str) {
    let mut offset = 0;
    while let Some(pos) = s[offset..].find(needle) {
...
fn offset_of_next_item_fish_2_0(
    ...
    while let Some(mut line) = lines.next() {
        ...
        *cursor = unsafe { offset(contents, next_line) };
